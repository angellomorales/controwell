...
public class CtrlOperacionTk implements ActionListener {
    ...
    public CtrlOperacionTk(FrmOperacionTk frmOperacionTk, OperacionTk operacionTk, Calculos calculos, ConsultasOperacionTk operacionTkC, Utilities ctrlUtilities, CtrlAforoTk ctrlAforoTk, CtrlTanques ctrlTanques, FrmApi frmApi, CalculoApi calculoApi, CtrlBalance ctrlBalance) {
        ...
        if (datosUltimoMov.getFecha() != null) {
            SpinnerDateModel horaModel = new SpinnerDateModel(datosUltimoMov.getHora(), null, null, Calendar.HOUR_OF_DAY);
...
//            this.frmOperacionTk.spnFecha.setModel(fechaModel);
            ...
            SpinnerDateModel horaModel = new SpinnerDateModel(this.ctrlUtilities.getFechaActual(), null, null, Calendar.HOUR_OF_DAY);
            this.frmOperacionTk.spnHora.setModel(horaModel);
...
//        this.frmOperacionTk.spnFecha.setEditor(ded);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == frmOperacionTk.btnGuardar) {
            ...
            Time hora = ctrlUtilities.getHoraActual();
            ...
            try {
                if (operacionTkC.buscarUltimoXPozoYTk(verificarOperacionTK)) {
                    if (operacionTk.getNivel() == verificarOperacionTK.getNivel()) {
                        ...
                        //fechaActual = ctrlUtilities.asignacionFechas((java.util.Date) frmOperacionTk.spnFecha.getValue(), (java.util.Date) frmOperacionTk.spnHora.getValue());
                        fechaActual = ctrlUtilities.asignacionFechas(frmOperacionTk.dtchFecha.getDate(), (java.util.Date) frmOperacionTk.spnHora.getValue());
                        fechaAnterior = ctrlUtilities.asignacionFechas(verificarOperacionTK.getFecha(), verificarOperacionTK.getHora());
                        //verificacion de fechas
                        this.verificacionFecha(fechaActual, fechaAnterior, hora, false);
                    }
                } else {
                    //asignacion de fechas
                    if (operacionTkC.buscarUltimaFecha(verificarOperacionTK)) {
                        ...
                        fechaAnterior = ctrlUtilities.asignacionFechas(verificarOperacionTK.getFecha(), verificarOperacionTK.getHora());
                        //verificacion de fechas
                        this.verificacionFecha(fechaActual, fechaAnterior, hora, true);
                    } else {
                ...
                if (this.ctrlBalance.balanceC.buscarUltimoXPozo(ultimoBal)) {
                    ...
                    if (this.operacionTkC.buscarUltimoXPozo(ultimaOp)) {
                        ...
                        if ("MENOR".equals(this.ctrlBalance.verificarDia(fechaUltimoBal, fechaUltimoMov))) {
                            seguro = JOptionPane.showConfirmDialog(null, "Seguro que desea eliminar el ultimo movimiento, ESTA OPCION NO SE PUEDE DESHACER");
                            if (seguro == 0) {
                            ...
                            JOptionPane.showMessageDialog(null, "no se puede eliminar el registro, verifique que no se haya generado el balance");
                        }
                    } else {
                        JOptionPane.showMessageDialog(null, "no se puede eliminar el registro, no se encuentra el ultimo movimiento");
                    }
                } else {
                    if (this.operacionTkC.buscaSiHayInicial(ultimaOp)) {
                        ...
                        JOptionPane.showMessageDialog(null, "no se puede eliminar el registro, no se encuentra el ultimo balance");
                    }
                }
    ...
    public void tomarDatos() {
        ...
        if ("Inicial".equals(tipo) || "Recibo".equals(tipo) || ("Transferencia".equals(tipo) && nivelAnterior < operacionTk.getNivel())) {
            ...
            operacionTk.setTempFluido(Double.parseDouble(ctrlUtilities.VerificarTextField(frmOperacionTk.txtTempFluido)));
            operacionTk.setTempAmbiente(Double.parseDouble(ctrlUtilities.VerificarTextField(frmOperacionTk.txtTempAmbiente)));
        } else {
            ...
            frmOperacionTk.txtTempFluido.setText(String.valueOf(calculos.formatearDecimales(operacionTk.getTempFluido(), 1)));
            frmOperacionTk.txtTempAmbiente.setText(String.valueOf(calculos.formatearDecimales(operacionTk.getTempAmbiente(), 1)));
        }
	...
	public void calcularVariables() {
        ...
        operacionTk.setCtsh(calculos.calcularCtsh(operacionTk.getTempFluido(), operacionTk.getTempAmbiente(), ctrlAforoTk.aforoTk.getTemperaturaBase(), ctrlAforoTk.aforoTk.getMaterial()));
        ...
        try {
            ...
            operacionTk.setCtl(calculos.calcularCTL(operacionTk.getApi(), operacionTk.getTempFluido(), ctrlTanques.tanques.getTipoFluido()));
...
//                    }
                ...
                JOptionPane.showMessageDialog(null, "Se registrara este movimiento como valor inicial, no se tendra en cuenta para el balance del dia");
    ...
    public void verificacionFecha(Calendar fechaActual, Calendar fechaAnterior, Time hora, boolean esInicial) {
        ...
        try {
            if (ctrlBalance.balanceC.buscarUltimo(verificarUltimoBalance)) {
                verificarFechaUltimoBalance = ctrlUtilities.asignacionFechas(verificarUltimoBalance.getFecha(), verificarUltimoBalance.getFecha());
            } else {
...
//            	if (fechaAnterior.getTimeInMillis() < fechaActual.getTimeInMillis()) {
                ...
                if (fechaAnterior.get(Calendar.DAY_OF_YEAR) == fechaActual.get(Calendar.DAY_OF_YEAR)) {
                    if (!ctrlBalance.hayBalance(fechaActual, operacionTk.getIdPozo())) {
                        ...
                        JOptionPane.showMessageDialog(null, "el balance ya fue realizado, no puede agregar mas movimientos al dia");
                    }
                } else {
                    if (ctrlBalance.hayBalance(fechaAnterior, operacionTk.getIdPozo())) {
                        if ("MENOR".equals(ctrlBalance.verificarDia(verificarFechaUltimoBalance, fechaActual))) {
                            ...
                            JOptionPane.showMessageDialog(null, "No se puede agregar el movimiento, verifique las fecha del ultimo balance generado ");
                        }
                    } else {
